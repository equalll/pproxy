...
// Global
func builtinGlobal_eval(call FunctionCall) Value {mydebug.INFO()
	src := call.Argument(0)
	if !src.IsString() {
		return src
	}
...
func builtinGlobal_isNaN(call FunctionCall) Value {mydebug.INFO()
	value := call.Argument(0).float64()
	return toValue_bool(math.IsNaN(value))
}

func builtinGlobal_isFinite(call FunctionCall) Value {mydebug.INFO()
	value := call.Argument(0).float64()
...
func digitValue(chr rune) int {mydebug.INFO()
	switch {
	case '0' <= chr && chr <= '9':
		return int(chr - '0')
	case 'a' <= chr && chr <= 'z':
...
func builtinGlobal_parseInt(call FunctionCall) Value {mydebug.INFO()
...
func builtinGlobal_parseFloat(call FunctionCall) Value {mydebug.INFO()
...
func _builtinGlobal_encodeURI(call FunctionCall, escape *regexp.Regexp) Value {mydebug.INFO()
...
func builtinGlobal_encodeURI(call FunctionCall) Value {mydebug.INFO()
	return _builtinGlobal_encodeURI(call, encodeURI_Regexp)
}

var encodeURIComponent_Regexp = regexp.MustCompile(`([^~!*()'])`)

func builtinGlobal_encodeURIComponent(call FunctionCall) Value {mydebug.INFO()
...
func _decodeURI(input string, reserve bool) (string, bool) {mydebug.INFO()
	if reserve {
...
func builtinGlobal_decodeURI(call FunctionCall) Value {mydebug.INFO()
...
func builtinGlobal_decodeURIComponent(call FunctionCall) Value {mydebug.INFO()
...
func builtin_shouldEscape(chr byte) bool {mydebug.INFO()
...
func builtin_escape(input string) string {mydebug.INFO()
...
func builtin_unescape(input string) string {mydebug.INFO()
...
func builtinGlobal_escape(call FunctionCall) Value {mydebug.INFO()
	return toValue_string(builtin_escape(call.Argument(0).string()))
}

func builtinGlobal_unescape(call FunctionCall) Value {mydebug.INFO()
